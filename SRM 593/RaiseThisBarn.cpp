...
class RaiseThisBarn {
    ...
    int calc(string str) {
        ...
        int forward[str.length()];
        memset(forward, 0, sizeof forward);

        FORN(i, str.length()) {
            if (i>0)
                forward[i] = forward[i-1] + (str[i] == 'c');
            else
                forward[i] = (str[i] == 'c');
        }

        int backward[str.length()];
        memset(backward, 0, sizeof backward);
        for(int i=str.length() - 1; i>=0; i--) {
            if (i < (str.length() - 1))
                backward[i] = backward[i+1] + (str[i] == 'c');
            else
                backward[i] = (str[i] == 'c');
        }
        ...
        FOR(i, 1, str.length() - 1) {
            int left = forward[i - 1];
            int right = backward[i];
            if (left == right)
                res++;
        }
        return res;
